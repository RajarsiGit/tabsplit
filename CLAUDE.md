...
**Vercel's Hobby plan caps a deployment at 12 Serverless Functions, and every file directly under `api/` counts as one — even pure helper modules with no `export default` handler.** Shared helpers that aren't themselves an endpoint (`db.js`, `balances.js`, `recurrence.js`) live in `api/_lib/` instead: Vercel ignores any file or directory under `api/` whose name starts with `_` when counting/deploying functions, so they're still importable via relative paths but don't burn a function slot. Keep new shared helpers in `api/_lib/`; only files meant to be routable endpoints belong directly in `api/`.
...
- `api/_lib/balances.js` — `computeBalances(sql, groupId)` (net balance per user, "paid" side summed from `expense_payments` — not `expenses.paid_by` — so multi-payer expenses attribute correctly; plus splits + settlements) and `simplifyDebts(balances)` (greedy min-transaction settle-up suggestions)
